				...
				<?php if (!isset( $_GET['from'] ) || $_GET['from'] !== self::LINK_POST_TYPE) { ?>
					if (typeof old_data !== 'undefined') {
						$.each(old_data, function (i, value) {
							...
							li += '<div class="detail"><div>（登録済み商品）</div><div class="title">';
							li += getTitleTagForSelect(value, true);
							li += '</div>';
							li += '<div class="button-box"><a class="button" href="<?php echo esc_url( admin_url() ); ?>post.php?post=' + yyiRinkerEscapeHTML( value['post_id'] ) + '&action=edit" rel="nofollow" target="_blank">リンク編集</a><button class="button button-primary select add-items-from-list" data-item-post-id="' + yyiRinkerEscapeHTML( value['post_id'] ) + '" >商品リンクを追加</button>';
					...
					if (typeof data !== 'undefined') {
                        ...
                        $.each(data, function (i, value) {
                            ...
                            li += '<div class="img"><img src="' + yyiRinkerEscapeHTML( value['<?php echo self::IMAGE_S_COLUMN ?>'] ) + '"></div>';
							li += '<div class="detail"><div class="title">';
							li += getTitleTagForSelect(value, false);
							li += '</div>';
                            ...
                            li += '<div class="button-box"><button class="button button-primary select add-items"';
							...
							<?php if ( isset($this->variant_images[$value[ 'key' ]]) ) { ?>
							var v_image_datas = JSON.stringify(value['<?php echo $value[ 'key' ] ?>']);
							var v_images = v_image_datas.replace(/"/g,"'");
							li += ' data-item-<?php echo esc_attr( $value[ 'key' ] ) ?>="' + v_images + '" ';
							<?php } else { ?>
...
if ( $value['is_ajax'] ) {?>
	...
	if (val !== 'undefined') {
	<?php if ( $value['key'] === self::S_VARIANT_IMAGES_COLUMN  || $value['key'] === self::M_VARIANT_IMAGES_COLUMN || $value['key'] === self::L_VARIANT_IMAGES_COLUMN) { ?>
		let image_datas =  element.data('item-<?php echo $value['key'] ?>');
		let image_data = image_datas.replace(/'/g,"\"");
		let v_image_datas =  JSON.parse(image_data);
		params['<?php echo $value['key'] ?>'] = v_image_datas;
	<?php } else { ?>
...
<?php if (isset( $_GET['from'] ) &&  $_GET['from'] === self::LINK_POST_TYPE) { ?>
            ...
            if (cid === '') {
				...
				$('div#widgets-editor', window.parent.document).find("div#block-" + cid ).find('.rinkerg-richtext').val(shortcode);
				$('div#widgets-editor', window.parent.document).find("div#block-" + cid ).find('.rinkerg-richtext').focus();
            }
...
<?php } else { ?>
			...
			$.ajax({
                ...
                if (cid === '') {
                    ...
                    $('div#editor', window.parent.document).find("div#block-" + cid ).find('.rinkerg-richtext').focus();
					$('div#widgets-editor', window.parent.document).find("div#block-" + cid ).find('.rinkerg-richtext').val(text);
					$('div#widgets-editor', window.parent.document).find("div#block-" + cid ).find('.rinkerg-richtext').focus();
                }
			}).always(function (jqXHR, textStatus) {
				window.parent.tb_remove();
...
<?php } ?>
		...
		function yyi_rinker_set_value_from_parent(params) {
			$.each(params, function( index, val ) {
				...
				switch ( index ) {
					...
					case '<?php echo self::S_VARIANT_IMAGES_COLUMN ?>':
					case '<?php echo self::M_VARIANT_IMAGES_COLUMN ?>':
					case '<?php echo self::L_VARIANT_IMAGES_COLUMN ?>':
						let textarea_metabox = $('#yyi_afilinks_middle_link textarea[name="' + index + '"]', parent.document);
						textarea_metabox.val(value);
						break;
					default :
		...
		function getTitleTagForSelect(value, is_icon) {
			let title = yyiRinkerEscapeHTML( value['<?php echo self::TITLE_COLUMN ?>'] );
			let icon = '';
			let title_url = '';
			if ( typeof value['<?php echo self::AMAZON_TITLE_URL_COLUMN  ?>'] !== 'undefined' && value['<?php echo self::AMAZON_TITLE_URL_COLUMN  ?>'] !== '') {
				title_url = value['<?php echo self::AMAZON_TITLE_URL_COLUMN  ?>'];
				//念の為トラッキングIDを削除
				title_url = yyiRinkerEscapeHTML(title_url.replace(/tag=.+?&/g, ""));
				icon = '<span class="yyi-rinker-shopicon yyi-rinker-amazon-icon">amazon</span>';
			} else if ( typeof value['<?php echo self::RAKUTEN_TITLE_URL_COLUMN  ?>'] !== 'undefined' && value['<?php echo self::RAKUTEN_TITLE_URL_COLUMN  ?>'] !== '') {
				title_url = yyiRinkerEscapeHTML( value['<?php echo self::RAKUTEN_TITLE_URL_COLUMN  ?>'] );
				icon = '<span class="yyi-rinker-shopicon yyi-rinker-rakuten-icon">楽天市場</span>';
			} else if  ( typeof value['<?php echo self::FREE_TITLE_URL_COLUMN  ?>'] !== 'undefined' && value['<?php echo self::FREE_TITLE_URL_COLUMN  ?>'] !== '') {
				title_url = yyiRinkerEscapeHTML( value['<?php echo self::FREE_TITLE_URL_COLUMN  ?>'] );
				icon = '<span class="yyi-rinker-shopicon yyi-rinker-free-icon">フリーリンク</span>';
			}
			if (!is_icon) {
				icon = '';
			}
			if (title_url === '') {
				return '<div class="yyi-rinker-title-link">' + title + '</div><div class="yyi-rinker-shopicon-box">' + icon + '</div>';
			} else {
				return '<div class="yyi-rinker-title-link"><a href="' + title_url +'" rel="nofollow noopener noreferrer" target="_blank">' + title + '</a></div><div class="yyi-rinker-shopicon-box">' + icon + '</div>';
			}
		}
