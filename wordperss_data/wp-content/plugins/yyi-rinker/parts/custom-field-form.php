		...
		<?php foreach($this->custom_field_params AS $index => $values) { ?>
			<div class="yyi-link-custom-field-item <?php echo $values[ 'is_link' ] ?  'relink' : ''; ?> <?php echo $values[ 'is_free' ] ?  'free' : 'not_free'; ?>">
				...
				<?php if ( self::SEARCH_SHOP_VALUE === $values[ 'key' ]) {
					...
					<?php } ?>
				<?php } elseif ( self::IS_AMAZON_NO_EXIST === $values[ 'key' ] || self::IS_RAKUTEN_NO_EXIST === $values[ 'key' ] || self::NO_RENEW === $values[ 'key' ]) { ?>
				...
				<?php } elseif ($values[ 'is_v_img' ] ) { ?>
					<textarea class="variant-images" id="<?php echo esc_attr( $values[ 'key' ] ); ?>" name="<?php echo esc_attr( $values[ 'key' ] ); ?>"><?php echo esc_textarea(get_post_meta($post->ID, $this->custom_field_column_name( $values[ 'key' ] ), true )) ?></textarea>
                <?php } else { ?>
