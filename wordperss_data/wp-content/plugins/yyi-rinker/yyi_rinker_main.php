	...
	public function image_html( $meta_datas, $shop_type, $atts = [] ) {
		...
		if ($this->is_some_images && is_array($v_images)) {
			...
			foreach($v_images AS $index => $v_image) {
				...
				$html .= '<img src="' . esc_url( $v_image->url ) . '" class="yyi-rinker-main-img skip-lazy ' . $img_class .'" style="border: none;"' . $some_lazyload . ' ' .  $size . '>';
			}
			if (count( $v_images ) > 0) {
			...
			if ($this->is_thumd($atts)) {
				...
				foreach ($v_images as $index => $v_image) {
					...
					$html .= '<li class="' . $li_class . '"><img src="' . esc_url($v_image->url) . '" class="skip-lazy" style="max-width: 30px; max-height: 30px; border: none;"' . $some_lazyload . '></li>';
				}
				$html .= '</ul>';
			}
		//画像一枚表示
	...
	function gutenberg_rinker_register_block()
	{
		...
		if (is_admin()) {
			...
			wp_register_script(
				...
				['wp-blocks', 'wp-i18n', 'wp-element', $editor, 'underscore'],
				getlastmod()
			);
		...
		register_block_type('rinkerg/gutenberg-rinker', [
			...
			'attributes' => [
				...
				'blockMarginBottom' => ['type' => 'string'],
				'hiddenSwitch' => ['type' => 'string'],
				'attention_text' =>  ['type' => 'string'],
				'attention_design' =>  ['type' => 'string'],
				'attention_color' =>  ['type' => 'string'],
				'templete_names' => ['type' => 'array']
			],
		...
		wp_add_inline_script(
			'gutenberg-rinker',
			sprintf(
				'var gutenberg_rinker = { localeData: %s, admin_url: %s , designs: %s, template_attrs: %s,templates: %s, template_max_count: %s };',
